...
u32_t _spiffs_erase_size;
u32_t _spiffs_phys_addr;
u32_t _spiffs_phys_size;
u32_t _spiffs_log_page_size;
u32_t _spiffs_log_block_size;
...
static int IRAM_ATTR vfs_spiffs_fstat(int fd, struct stat * st) {
	...
	st->st_blksize = _spiffs_log_page_size;// CONFIG_SPIFFS_LOG_PAGE_SIZE;
...
int IRAM_ATTR spiffs_format() {
	ESP_LOGI(tag, "Formatting");
	SPIFFS_unmount(&fs);
	auto res = SPIFFS_format(&fs);
	if (res < 0) {
...
int IRAM_ATTR spiffs_mount() {
    ...
    cfg.phys_addr 		 = _spiffs_phys_addr;//CONFIG_SPIFFS_BASE_ADDR;
    cfg.phys_size 		 = _spiffs_phys_size;//CONFIG_SPIFFS_SIZE;
	cfg.phys_erase_block = _spiffs_erase_size;// SPIFFS_ERASE_SIZE;
    cfg.log_page_size    = _spiffs_log_page_size;//CONFIG_SPIFFS_LOG_PAGE_SIZE;
    cfg.log_block_size   = _spiffs_log_block_size;//CONFIG_SPIFFS_LOG_BLOCK_SIZE;
...
void IRAM_ATTR vfs_spiffs_register(uint32_t phys_addr, uint32_t phys_size, uint32_t log_page_size, uint32_t log_block_size, uint32_t erase_size) {
	...
	_spiffs_log_page_size = log_page_size;
	_spiffs_log_block_size = log_block_size;
...
//=============================
int IRAM_ATTR spiffs_unmount(int unreg) {
