...
void IRAM_ATTR _mtx_init() {
}

void IRAM_ATTR mtx_init(struct mtx *mutex, const char *name, const char *type, int opts) {
...
int IRAM_ATTR mtx_trylock(struct	mtx *mutex) {
...
void IRAM_ATTR mtx_destroy(struct	mtx *mutex) {
    if (port_interruptNesting[xPortGetCoreID()] != 0) {
        BaseType_t xHigherPriorityTaskWoken = pdFALSE;  
        xSemaphoreGiveFromISR( mutex->sem, &xHigherPriorityTaskWoken );  
        portEND_SWITCHING_ISR( xHigherPriorityTaskWoken );
