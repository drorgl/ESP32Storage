...
u8_t IRAM_ATTR SPIFFS_mounted(spiffs *fs) {
  return SPIFFS_CHECK_MOUNT(fs);
}

s32_t IRAM_ATTR SPIFFS_format(spiffs *fs) {
#if SPIFFS_READ_ONLY
  (void)fs;
...
s32_t IRAM_ATTR SPIFFS_probe_fs(spiffs_config *config) {
...
s32_t IRAM_ATTR SPIFFS_mount(spiffs *fs, spiffs_config *config, u8_t *work,
...
void IRAM_ATTR SPIFFS_unmount(spiffs *fs) {
...
s32_t IRAM_ATTR SPIFFS_errno(spiffs *fs) {
  return fs->err_code;
}

void IRAM_ATTR SPIFFS_clearerr(spiffs *fs) {
  fs->err_code = SPIFFS_OK;
}

s32_t IRAM_ATTR SPIFFS_creat(spiffs *fs, const char *path, spiffs_mode mode) {
#if SPIFFS_READ_ONLY
...
spiffs_file IRAM_ATTR SPIFFS_open(spiffs *fs, const char *path, spiffs_flags flags, spiffs_mode mode) {
  (void)mode;
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  if (strlen(path) > SPIFFS_OBJ_NAME_LEN - 1) {
...
spiffs_file IRAM_ATTR SPIFFS_open_by_dirent(spiffs *fs, struct spiffs_dirent *e, spiffs_flags flags, spiffs_mode mode) {
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
spiffs_file IRAM_ATTR SPIFFS_open_by_page(spiffs *fs, spiffs_page_ix page_ix, spiffs_flags flags, spiffs_mode mode) {
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
static s32_t IRAM_ATTR spiffs_hydro_read(spiffs *fs, spiffs_file fh, void *buf, s32_t len) {
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
s32_t IRAM_ATTR SPIFFS_read(spiffs *fs, spiffs_file fh, void *buf, s32_t len) {
...
#if !SPIFFS_READ_ONLY
static s32_t IRAM_ATTR spiffs_hydro_write(spiffs *fs, spiffs_fd *fd, void *buf, u32_t offset, s32_t len) {
  (void)fs;
  s32_t res = SPIFFS_OK;
  s32_t remaining = len;
...
s32_t IRAM_ATTR SPIFFS_write(spiffs *fs, spiffs_file fh, void *buf, s32_t len) {
#if SPIFFS_READ_ONLY
...
s32_t IRAM_ATTR SPIFFS_lseek(spiffs *fs, spiffs_file fh, s32_t offs, int whence) {
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
s32_t IRAM_ATTR SPIFFS_remove(spiffs *fs, const char *path) {
#if SPIFFS_READ_ONLY
  (void)fs; (void)path;
  return SPIFFS_ERR_RO_NOT_IMPL;
#else
...
s32_t IRAM_ATTR SPIFFS_fremove(spiffs *fs, spiffs_file fh) {
#if SPIFFS_READ_ONLY
  (void)fs; (void)fh;
  return SPIFFS_ERR_RO_NOT_IMPL;
#else
...
static s32_t IRAM_ATTR spiffs_stat_pix(spiffs *fs, spiffs_page_ix pix, spiffs_file fh, spiffs_stat *s) {
  (void)fh;
...
s32_t IRAM_ATTR SPIFFS_stat(spiffs *fs, const char *path, spiffs_stat *s) {
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  if (strlen(path) > SPIFFS_OBJ_NAME_LEN - 1) {
    SPIFFS_API_CHECK_RES(fs, SPIFFS_ERR_NAME_TOO_LONG);
...
s32_t IRAM_ATTR SPIFFS_fstat(spiffs *fs, spiffs_file fh, spiffs_stat *s) {
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
#if SPIFFS_CACHE == 1
static s32_t IRAM_ATTR spiffs_fflush_cache(spiffs *fs, spiffs_file fh) {
  (void)fs;
  (void)fh;
  s32_t res = SPIFFS_OK;
...
s32_t IRAM_ATTR SPIFFS_fflush(spiffs *fs, spiffs_file fh) {
  (void)fh;
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  s32_t res = SPIFFS_OK;
...
s32_t IRAM_ATTR SPIFFS_close(spiffs *fs, spiffs_file fh) {
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
...
s32_t IRAM_ATTR SPIFFS_rename(spiffs *fs, const char *old_path, const char *new_path) {
#if SPIFFS_READ_ONLY
...
#if SPIFFS_OBJ_META_LEN
s32_t IRAM_ATTR SPIFFS_update_meta(spiffs *fs, const char *name, const void *meta) {
#if SPIFFS_READ_ONLY
...
s32_t IRAM_ATTR SPIFFS_fupdate_meta(spiffs *fs, spiffs_file fh, const void *meta) {
#if SPIFFS_READ_ONLY
...
spiffs_DIR IRAM_ATTR *SPIFFS_opendir(spiffs *fs, const char *name, spiffs_DIR *d) {
  (void)name;
...
static s32_t IRAM_ATTR spiffs_read_dir_v(
    spiffs *fs,
    spiffs_obj_id obj_id,
    spiffs_block_ix bix,
    int ix_entry,
...
struct spiffs_dirent IRAM_ATTR *SPIFFS_readdir(spiffs_DIR *d, struct spiffs_dirent *e) {
...
s32_t IRAM_ATTR SPIFFS_closedir(spiffs_DIR *d) {
  SPIFFS_API_CHECK_CFG(d->fs);
  SPIFFS_API_CHECK_MOUNT(d->fs);
  return 0;
}

s32_t IRAM_ATTR SPIFFS_check(spiffs *fs) {
#if SPIFFS_READ_ONLY
  (void)fs;
  return SPIFFS_ERR_RO_NOT_IMPL;
#else
...
s32_t IRAM_ATTR SPIFFS_info(spiffs *fs, u32_t *total, u32_t *used) {
  s32_t res = SPIFFS_OK;
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
s32_t IRAM_ATTR SPIFFS_gc_quick(spiffs *fs, u16_t max_free_pages) {
#if SPIFFS_READ_ONLY
...
s32_t IRAM_ATTR SPIFFS_gc(spiffs *fs, u32_t size) {
#if SPIFFS_READ_ONLY
  (void)fs; (void)size;
  return SPIFFS_ERR_RO_NOT_IMPL;
#else
...
s32_t IRAM_ATTR SPIFFS_eof(spiffs *fs, spiffs_file fh) {
  s32_t res;
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
s32_t IRAM_ATTR SPIFFS_tell(spiffs *fs, spiffs_file fh) {
  s32_t res;
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
s32_t IRAM_ATTR SPIFFS_set_file_callback_func(spiffs *fs, spiffs_file_callback cb_func) {
  SPIFFS_LOCK(fs);
...
s32_t IRAM_ATTR SPIFFS_ix_map(spiffs *fs,  spiffs_file fh, spiffs_ix_map *map,
...
s32_t IRAM_ATTR SPIFFS_ix_unmap(spiffs *fs,  spiffs_file fh) {
  s32_t res;
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
s32_t IRAM_ATTR SPIFFS_ix_remap(spiffs *fs, spiffs_file fh, u32_t offset) {
  s32_t res = SPIFFS_OK;
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
...
s32_t IRAM_ATTR SPIFFS_bytes_to_ix_map_entries(spiffs *fs, u32_t bytes) {
  SPIFFS_API_CHECK_CFG(fs);
...
s32_t IRAM_ATTR SPIFFS_ix_map_entries_to_bytes(spiffs *fs, u32_t map_page_ix_entries) {
  SPIFFS_API_CHECK_CFG(fs);
...
s32_t IRAM_ATTR SPIFFS_vis(spiffs *fs) {
  s32_t res = SPIFFS_OK;
  SPIFFS_API_CHECK_CFG(fs);
  SPIFFS_API_CHECK_MOUNT(fs);
  SPIFFS_LOCK(fs);
